...
//=======================================================================
// AAHFT v2 — TUNING NOTES (on top of AAHFT.mq5 / original v2 "Including new Trailing Stops")
...
// - NEW in v2: session profit/loss lock (see SESSION PROFIT/LOSS LOCK group
//   below). Once account balance reaches ProfitTargetBalance or falls to
//   MaxLossBalance, the EA immediately closes every open position, cancels
//   every pending order, and stops opening new ones for the rest of this
//   run/session. This exists because backtesting found that RiskPercent-based
//   compounding can turn a near-breakeven edge into a severe boom-then-crash
//   swing (see EA/gold-xauusd-exploration/findings.md, the G1/G2/G2b entries)
//   — this lock is what makes "stop once we're up $X" or "cap losses at $Y"
//   an enforced rule instead of something a human has to react to in time,
//   which isn't realistic at this EA's trading frequency (seconds per trade).
// - NEW in v2: default LotType changed from Fixed_Lots to Pct_of_Balance
//   (RiskPercent=1), since testing found fixed-lot sizing doesn't scale to
//   small accounts and can produce single trades costing 20%+ of a $100
//   balance (see EA/gold-xauusd-exploration/findings.md, gold_4.xlsx).
...
input group  "SESSION PROFIT/LOSS LOCK"; // Secure wins / cap losses, enforced automatically
      input double ProfitTargetBalance = 5000; // Flatten & stop once BALANCE reaches this level (0 = disabled)
      input double MaxLossBalance = 50; // Flatten & stop if BALANCE falls to this level (0 = disabled)

input group "MONEY MANAGEMENT"; // MONEY MANAGEMENT

      input enumLotType LotType = 1; // Type of Lotsize calculation
...
// Session profit/loss lock: once tripped, no more new orders for the rest
// of this run — reset automatically whenever the EA is (re)initialized
// (a fresh backtest run, or reattaching live with a fresh deposit).
bool SessionLocked;
...
// the hour-of-day gate, the weekend-gap guard, and the session profit/loss
// lock.
...
// Checks the session profit/loss lock against current balance, and if it
// trips, immediately cancels every pending order and closes every open
// position for this EA/symbol. This is the enforcement mechanism for
// "secure the win" / "cap the loss" — it does not wait for positions to
// close naturally via their own stop-loss.
void CheckSessionLock()
{
   if(SessionLocked) return;

   double bal = AccountInfoDouble(ACCOUNT_BALANCE);
   bool hitProfitTarget = (ProfitTargetBalance > 0 && bal >= ProfitTargetBalance);
   bool hitLossFloor = (MaxLossBalance > 0 && bal <= MaxLossBalance);

   if(!hitProfitTarget && !hitLossFloor) return;

   SessionLocked = true;
   Print("AAHFT: session lock triggered at balance ", bal,
         hitProfitTarget ? " (profit target reached)" : " (loss floor reached)",
         " - cancelling pending orders and closing open positions.");
